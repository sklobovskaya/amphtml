...
import {AmpGwdRuntimeService, GWD_SERVICE_NAME, GWD_TIMELINE_EVENT}
...
import {getParentWindowFrameElement, getServiceForDoc}
  from '../../../src/service';
...
export class GwdAnimation extends AMP.BaseElement {
  ...
  constructor(element) {
    ...
    /**
     * The friendly iframe embed, if in one. If in a single AmpDoc, this will
     * remain null.
  ...
  buildCallback() {
    ...
    // Install the runtime. The page deck element is expected to be already
    // present in the DOM.
    const service = user().assert(
        getServiceForDoc(this.getAmpDoc(), GWD_SERVICE_NAME),
        'Cannot initialize GWD service because it is not registered.');
  ...
  executeInvocation_(invocation) {
    ...
    // Prepend the document root to the parameter list.
    actionArgs.unshift(this.getRoot_());
 ...
 *     than a document is provided when in a FIE.
