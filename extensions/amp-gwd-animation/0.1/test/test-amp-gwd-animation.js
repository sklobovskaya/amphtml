...
describes.sandboxed('AMP GWD Animation', {}, () => {
  ...
  describes.repeated('', {
    ...
    describes.realWin('', {
      ...
      /**
       * Returns the top-level or FIE window, depending on the AmpDoc variant
       * used in the test.
       * @return {!Window}
       */
      function getLocalWin() {
        return variant.ampdoc == 'fie' ? embed.win : ampdoc.win;
      }

      /**
       * Returns the top-level or FIE document, depending on the AmpDoc variant
       * used in the test.
       * @return {!Document}
       */
      function getLocalDoc() {
        return variant.ampdoc == 'fie' ?
          embed.win.document :
          ampdoc.getRootNode();
      }
      ...
      beforeEach(() => {
        ...
        return createGwdAnimationElement(doc, config)
            .then(el => {
              ...
              page1Elem = getLocalDoc().getElementById('page1');
            });
      });

      afterEach(() => {
        getLocalDoc().body.innerHTML = '';
      });
      ...
      it('should immediately disable animations', () => {
        return ampdoc.whenBodyAvailable().then(() => {
          const body = getLocalDoc().body;
          expect(body.classList.contains(ANIMATIONS_DISABLED_CLASS)).to.be.true;
        });
      });
      */
      ...
      it('should install slideChange listeners on the GWD pagedeck', () => {
        const pagedeck = getLocalDoc().getElementById(GWD_PAGEDECK_ID);
      ...
      it('should activate and deactivate pages', () => {
        ...
        // Activate page 1.
        runtime.setCurrentPage(getLocalDoc(), 0);
        ...
        // Change to page 2.
        runtime.setCurrentPage(getLocalDoc(), 1);
      ...
      it('should execute gotoAndPlayNTimes', () => {
        const testEvent = createCustomEvent(
            getLocalWin(), GWD_TIMELINE_EVENT, {'eventName': 'event-1'});
      ...
      it('should get the receiver element by id if it exists', () => {
        ...
        expect(runtime.getReceiver(doc, 'document.body')).to.equal(doc.body);
        expect(runtime.getReceiver(doc, 'page1')).to.equal(page1Elem);
        expect(runtime.getReceiver(doc, 'nonexistentElement')).to.be.null;
      });
    });
  });
